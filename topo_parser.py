        ...
        for i, line in enumerate(lines):
            ...
            if b:
                try:
                    ...
                    name = split_string[3].replace('"', '')
                    name_split = name.split(" ")
                    result = name_split[1]

                    topology[(switch_name, switch_number, result)] = current_switch
                    # Print the progress
        ...
        if len(sys.argv) == 2:
            if sys.argv[1] == "-h":
                print("You should run the command 'topo_parser –f topofile.topo' to parse your file.\n"
                      "After it, a parsed file named parse.txt is being created. with the format a,b,c,d for each "
                      "line\n"
                      "when 'a' is device, 'b' is the port, 'c' is the name of the host/switch.\n'd' is the switch "
                      "that the device is "
                      "connected to.\n"
